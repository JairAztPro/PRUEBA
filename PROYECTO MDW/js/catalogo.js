...
/**
 * ============================================================================
 * SISTEMA DE PAGINACIÓN DINÁMICA
 * ============================================================================
 */
const ITEMS_POR_PAGINA = 3; // Modifica este valor según cuántos productos quieras por vista
let paginaActual = 1;

function actualizarPaginacionYVisibilidad() {
  const productos = [...document.querySelectorAll(".producto")];
  
  // Filtrar solo los elementos que pasaron los filtros de búsqueda, marca, precio y stock
  const productosVisibles = productos.filter(producto => {
    return !producto.classList.contains("d-none-por-filtro"); // o tu lógica actual de clases
  });

  const totalPaginas = Math.ceil(productos.length / ITEMS_POR_PAGINA) || 1;
  if (paginaActual > totalPaginas) paginaActual = totalPaginas;

  // Controlar qué productos se muestran según la página activa
  const inicio = (paginaActual - 1) * ITEMS_POR_PAGINA;
  const fin = inicio + ITEMS_POR_PAGINA;

  productos.forEach((producto, index) => {
    // Verificamos si pasa los filtros actuales primero
    const estaFiltrado = producto.classList.contains("d-none"); // Asumiendo que tu filtro usa d-none
  ...
  // Renderizar los botones de paginación dinámicamente en el DOM
...
window.cambiarPagina = function(nuevaPagina) {
  ...
  if (nuevaPagina >= 1 && nuevaPagina <= totalPaginas) {
    ...
    actualizarPaginacionYVisibilidad();
